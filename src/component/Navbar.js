...
import {setIndexWallet, getWallets} from "../service/wallet/walletService";
...
export default function Navbar({wallets, selectedWalletIndex}) {
    ...
    useEffect(() => {
        if (showWallet) {
            dispatch(getDetails(showWallet.id))
        }
    ...
    return (<>
            ...
            <div className="lib-item-tab-left" onClick={handleClick}>
                {showWallet && (<>
                    ...
                    <div className="content-wallet">
                        ...
                        <span>{formatMoney(showWallet.money)}</span>
        ...
        <div className={`form-sidebar ${isShow ? 'show' : ''}`}>
            ...
            {wallets.map((wallet, index) => (<div key={wallet.id} className="included-from-total-wallet">
                <div className="wallet-img p-2">
                    <img className="img-show-wallet" src={wallet.avatar} alt=""/>
                </div>
                <div className="wallet-info p-2 flex-grow-1" onClick={() => {
                    choiceWallet(index)
                }}>
